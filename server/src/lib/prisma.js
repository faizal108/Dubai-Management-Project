...
// mutated), so the update-side branches below skip it via the audit set.
...
// from AUDITED_MODELS so the existing update/upsert branches don't touch it.
const CREATE_ONLY_AUDIT_MODELS = new Set(["Transaction"]);
    ...
    query: {
      $allModels: {
        ...
        async create({ model, args, query }) {
          ...
          } else if (CREATE_ONLY_AUDIT_MODELS.has(model)) {
            args.data = { createdBy: actor(), ...args.data };
          }
          return query(args);
        },
        async createMany({ model, args, query }) {
          ...
          } else if (CREATE_ONLY_AUDIT_MODELS.has(model) && Array.isArray(args.data)) {
            args.data = args.data.map((d) => ({ createdBy: actor(), ...d }));
          }
          return query(args);
        },
