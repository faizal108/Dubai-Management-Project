...
// Maps a Prisma P2002 (unique-constraint) error on the donor phone partial
// index onto a friendly 409. Any other P2002 target falls through to the
// generic handler so we don't mask unrelated conflicts.
function handleDonorUniqueError(err) {
  if (err?.code !== "P2002") return err;
  const target = err.meta?.target;
  const fields = Array.isArray(target) ? target : typeof target === "string" ? [target] : [];
  const onPhone = fields.some((t) => String(t).includes("phone"));
  const onPan = fields.some((t) => String(t).includes("pan"));
  if (onPhone) {
    return ApiError.conflict("Another donor with this mobile number already exists in this foundation");
  }
  if (onPan) {
    return ApiError.conflict("Donor with this PAN already exists in this foundation");
  }
  return err;
}
...
export async function createDonor(user, input) {
  ...
  // PAN is optional in the 3-tier model, so the pre-flight check only runs
  // when a PAN was actually provided. NULLs are Postgres-distinct so the
  // underlying unique index doesn't collide on absent values.
  if (input.pan) {
  ...
  try {
    ...
    // Race window between the PAN pre-check and insert, plus the phone
    // partial-unique index which we don't pre-check.
    throw handleDonorUniqueError(err);
  }
}
