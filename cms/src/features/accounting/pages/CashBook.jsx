// src/features/accounting/pages/CashBook.jsx
// Chronological ledger for cash-only accounts (BankAccount.accountNumber IS
// NULL). Rows are ordered occurredAt ASC so the `balanceAfter` column reads
// as a running balance from opening to close.
...
const CashBook = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (to) params.to = to;
      const res = await listCashBook(params);
      setData({
        items: res?.items ?? [],
        total: res?.total ?? 0,
      ...
      },
      {
        ...
        cell: (r) =>
          r.type === "CREDIT" ? (
            ...
            <span className="rounded bg-rose-500/10 px-1.5 py-0.5 text-xs font-semibold text-rose-600 dark:text-rose-400">
              Debit
            </span>
          ),
      },
  ...
  return (
    <>
      ...
      <LedgerTable
        ...
        emptyDescription="No cash-account transactions match the current filter window."
      />
    </>
  );
};
