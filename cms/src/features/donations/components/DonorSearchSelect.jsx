...
// the foundation's roster by name or PAN; results are debounced against the
// list endpoint and rendered as "PAN — Name". The selected donor object is
// lifted to the parent via onSelect.
...
const DonorSearchSelect = ({ donor, onSelect, disabled, fieldError }) => {
  ...
  // Once a donor is selected the input displays "PAN — Name"; while the user
  // is typing, the live query takes over until selection or blur.
  const displayValue = (d) => (d ? `${d.pan} — ${d.fullName}` : "");

  return (
    <div className="md:col-span-2">
      <FormField label="Donor" required error={fieldError}>
                    ...
                    {({ selected }) => (
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <div className="truncate">{d.fullName}</div>
                        </div>
                        {selected && (
